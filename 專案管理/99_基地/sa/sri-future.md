...
| v4 驗證腳本自動化 | 目前仍以 `rg` / `grep` 人工驗證為主 | `99_基地/mcp/` 或外部腳本 |
| MCP server 預設庫 | 需要累積多個穩定 server 樣板 | `99_基地/mcp/servers.json` |
...
- 是否需要為 `_templates/` 增加第三種大型企業專案樣板。
