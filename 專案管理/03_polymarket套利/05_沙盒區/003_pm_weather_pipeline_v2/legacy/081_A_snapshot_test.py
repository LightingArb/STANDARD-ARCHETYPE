"""
081_A_snapshot_test.py — A 源 (IEM/METAR) snapshot 正式入口

兩個 phase：
  Phase 1: availability probe — 確認 city × station 的 IEM 可用性
  Phase 2: snapshot batch fetch — 依 probe 結果抓取 hourly 觀測資料

語意說明：
  A 源是氣象站觀測資料（METAR/ASOS），不是預報也不是 reanalysis。
  snapshot_date = 那天的觀測紀錄起始。
  horizon_hour = target_time 相對 snapshot_date 00:00 的偏移。
  用途：與 C/D 對齊，提供實際觀測溫度作為 ground truth。

實作邏輯：
  _lib/a_snapshot_probe.py (Phase 1)
  _lib/a_snapshot_fetch.py (Phase 2)

輸出路徑：
  Probe:  logs/08_snapshot/probe_a/
  Fetch:  08_snapshot/A/ (data), logs/08_snapshot/fetch_a/ (logs)

用法：
  python 081_A_snapshot_test.py
  python 081_A_snapshot_test.py --cities London --start-date 2026-04-01 --end-date 2026-04-02
  python 081_A_snapshot_test.py --skip-probe   (跳過 Phase 1，直接用上次 probe 結果)
"""
...
PROBE_OUTPUT_DIR = PROJ_DIR / "logs" / "08_snapshot" / "probe_a"
FETCH_OUTPUT_ROOT = PROJ_DIR / "08_snapshot" / "A"
FETCH_LOG_DIR = PROJ_DIR / "logs" / "08_snapshot" / "fetch_a"
DEFAULT_CITY_CSV = PROJ_DIR / "data" / "01_city.csv"


def main():
    parser = argparse.ArgumentParser(
        description="A-source snapshot test (Phase 1: probe + Phase 2: fetch)"
    )
    parser.add_argument(
    ...
    if not args.skip_probe:
        ...
        from _lib_legacy.a_snapshot_probe import run_probe
    ...
    from _lib_legacy.a_snapshot_fetch import run_fetch
    ...
    if exit_code == 0:
        ...
        if jobs:
            log.info(
                f"{'city':<18} {'station':<10} {'status':<22} "
    ...
    log.info("=" * 80)
    log.info("081_A_snapshot_test: complete")
